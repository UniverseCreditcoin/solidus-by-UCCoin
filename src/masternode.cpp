...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if(nHeight > 300000) return 8000 * COIN;
    if(nHeight > 200000) return 4000 * COIN;
    if(nHeight > 151000) return 2000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 SLDS tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 SLDS tx -> 1 confirmation
